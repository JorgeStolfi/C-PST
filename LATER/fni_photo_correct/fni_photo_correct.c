...
#define PROG_DESC "computes a camera's light transfer table from a photo of a gray scale"
#define PROG_VERS "1.0"

/* Copyright © 2004 by the State University of Campinas (UNICAMP).*/
/* See the copyright, authorship, and warranty notice at end of file.*/
/* Last edited on 2015-10-18 02:58:01 by stolfilocal */

#define PROG_HELP \
  PROG_NAME "\\\n" \
  "    -grayScale {GRAY_SCALE_FNI} \\\n" \
  "    [ -refStrip {REF_STRIP_FNI} ] \\\n" \
  "    [ -useSelf ] \\\n" \
...
#define PROG_INFO \
  ...
  "\n" \
  "DESCRIPTION\n" \
  "  Reads digital digital photograph of a gray-scale reflectance strip" \
  " (such as the one provided by the Kodak Q-13 grayscale chart), and" \
  " estimates from it the camera's /light transfer function/ (LTF), the" \
  " correspondence between the physical lightness (brightness) of a surface" \
  " to the sample values stored in a digital photo of the same.  The LTF is written out in tabular or analytic form.  The program may also output a photometrically corrected copy of the chart.\n" \
  "\n" \
  "\n" \
  "  The scene's illumination does not have to be uniform" \
  " over the whole chart, as long as it has negligible" \
  " variation over any 3 × 3 mm region on the chart.  However, any" \
  " variations of apparent brightness due to varying" \
  " illumination will be retained in the output image.\n" \
  "\n" \
  "  Optionally, the program writes to the text file \"{ADJUST_TBL_FNI}\"" \
  " the correspondence between input sample values and output" \
  " intensities, in a human- and machine-readable format.  In addition," \
  " the program may write to file \"{BASIS_FNI}\" the basis functions" \
  " used for fitting.\n" \
  "\n" \
  "  The program may also write to the files \"{RAW_CHART_FNI}\" and" \
  " \"{ADJ_CHART_FNI}\" two images of the Q-13 chart" \
  " only, extracted from the input image and geometrically" \
  " corrected to true-scale projection, respectively before and after" \
  " the photometric correction.  In addition, the narrow strips" \
  " of the same charts, containing the information used by" \
  " the program, may be written out to \"{RAW_STRIP_FNI}\" and" \
  " \"{ADJ_STRIP_FNI}\".\n" \
  "\n" \
  ...
  "\n" \
  "REFERENCE AREAS\n" \
  "  Depending on the options, the program may use the strip of Q-13's" \
  " gray background adjacent to the gray scale, and/or a strip just below" \
  " the scale (assumed to be in a mounting frame of uniform" \
  " color).  Therefore, those strips, as well as the gray" \
  " scale itself, must be entirely visible, and free from" \
  " defects such as dirt, smudges, projected shadows, specular" \
  " reflections, etc..\n" \
  "\n" \
  "GENERAL OPTIONS\n" \
  "  " fka_corners_HELP " \n" \
  ...
  "\n" \
  "SEE ALSO\n" \
  "  pnm_to_fni(1), fni_to_pnm(1), fni_hist(1), pnmnorm(1).\n" \
  "\n" \
  "AUTHOR\n" \
...
typedef pst_Kodak_Q13_basis_t fka_basis_t; /* A shorter name. */

#define fka_default_basis_type pst_Kodak_Q13_btype_A
  ...
  { 
    ...
    for (c = 0; c < NC; c++)
      { coef[c] = double_vec_new(o->basis.N);
        pst_Kodak_Q13_compute_light_map
          ( IMG, c, noise, o->useGray, o->useWhite, o->useSelf, &P, 
            o->basis, o->monotonic,
            coef[c].e, &(logVlo[c]), &(logVhi[c])
          );
        pst_Kodak_Q13_apply_map(IMG, c, noise, logVlo[c], logVhi[c], o->basis, coef[c].e);
      }
...
void fka_write_chart_and_strip
  ...
  )
  {
    ...
    if (chartName != NULL)
      { float_image_t *QIMG = pst_Kodak_Q13_extract_chart(IMG, P, step); 
    ...
    if (stripName != NULL)
      { float_image_t *SIMG = 
          pst_Kodak_Q13_extract_chart_strips(IMG, P, useGray, useWhite, step); 
  ...
  { float_image_t *TIMG = float_image_new(NC+1, NP, 1);
    ...
    for (i = 0; i < NP; i++)
      { double V = i/((double)NP-1);
        ...
        int c;
        for (c = 0; c < NC; c++)
          { double Vc = pst_Kodak_Q13_eval_map(V, noise, logVlo[c], logVhi[c], B, coef[c].e);
  ...
  { int K = NP/20; /* Points outside the {[0_1]} range, at each end. */
    ...
    for (i = 0; i < NP; i++)
      { double v = (i - K)/((double)NP-2*K-1);
        ...
        int c;
        for (c = 0; c < NC; c++)
          { double vc = pst_Kodak_Q13_eval_raw_map(v, B, coef[c].e);
            float_image_set_sample(TIMG, c+1, i, 0, vc);
          }
      }
    fka_write_float_image(name, TIMG);
  ...
  { 
    ...
    for (i = 0; i < NP; i++)
      { double v = (i-KP)/((double)NP-2*KP-1);
        ...
        int p;
        for (p = 0; p < NB; p++)
          { double zp = pst_Kodak_Q13_eval_basis(v, p, B);
  ...
  { 
    ...
    if (argparser_keyword_present(pp, "-basis"))
      { if (argparser_keyword_present_next(pp, "A"))
          { o->basis.bt = pst_Kodak_Q13_btype_A; }
        else if (argparser_keyword_present_next(pp, "B"))
          { o->basis.bt = pst_Kodak_Q13_btype_B; }
        else 
