#! /bin/bash
# Last edited on 2025-01-09 01:55:16 by stolfi
...
out_weights_fni="${outPrefix}-W.fni"
...
if [[ ${view_fni} -gt 0 ]]; then
  ...
  ofile=${out_weights_fni}; if [[ -s ${ofile} ]]; then fni_view -channel 0 -scale ${scale} ${ofile}; fi
...
if [[ ${make_pnm} -gt 0 ]]; then
  ...
  ofile=${out_weights_fni}; if [[ -s ${ofile} ]]; then 
    pfile="${ofile/.fni/.pgm}"
    fni_to_pnm -channel 0 -min 0 -max 1 < ${ofile} > ${pfile}
    if [[ -s ${pfile} ]]; then pfiles+=( ${pfile} ); fi
  fi
...
if [[ ${make_plot} -gt 0 ]]; then
  ...
  ofile=${out_weights_fni}; if [[ -s ${ofile} ]]; then 
    efile="${ofile/.fni/.eps}"; efiles+=( ${efile} )
    fni_plot.sh -channel 0 -title "W" < ${ofile} > ${efile}
    if [[ -s ${efile} ]] ; then efiles+=( ${efile} ); fi
  fi
...
if [[ ${make_hist} -gt 0 ]]; then
  ...
  ofile=${out_weights_fni}; if [[ -s ${ofile} ]]; then 
    efile_h="${ofile/.fni/-h.eps}"
    fni_hist -channel 0 -title "W" -step 0.250 < ${ofile} > ${efile_h}
    if [[ -s ${efile_h} ]] ; then hfiles+=( ${efile_h} ); fi
  fi
fi
